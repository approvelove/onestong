...
- (void)setCustomEditing:(BOOL)customEditing
{
    if (_customEditing != customEditing) {
        ...
        self.OSTTableView.scrollEnabled = !customEditing;
        if (customEditing) {
            if (!_overlayView) {
...
//                _overlayView.alpha = 0.4;
            ...
            if (self.shouldDisableUserInteractionWhileEditing) {
                for (UIView *view in self.OSTTableView.subviews) {
...
- (UIView *)overlayView:(DAOverlayView *)view didHitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    CGPoint location = [self.OSTTableView convertPoint:point fromView:view];
    CGRect rect = [self.OSTTableView convertRect:self.cellDisplayingMenuOptions.frame toView:self.OSTTableView];
